...
parser.add_argument('-j', '--workers', type=int, default=24, metavar='N',
...
def main():
    ...
    if args.rank == 0:
        ...
        if SummaryWriter is None:
            _logger.warning('TensorBoard is not available. Install it with: pip install tensorboard')
        else:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if not args.speedtest and is_eval:
                ...
                if tb_writer is not None:
                    tb_writer.add_scalar('val/top1', eval_metrics['top1'], epoch)
                    tb_writer.add_scalar('val/top5', eval_metrics['top5'], epoch)
                    tb_writer.add_scalar('val/ce_loss', eval_metrics['loss'], epoch)
                    if 'dist_loss' in eval_metrics:
                        tb_writer.add_scalar('val/dist_loss', eval_metrics['dist_loss'], epoch)
                    if 'kd_loss' in eval_metrics:
                        tb_writer.add_scalar('val/kd_loss', eval_metrics['kd_loss'], epoch)
                    tb_writer.flush()
